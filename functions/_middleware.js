...
const BRAND_NAME  = 'PJ Antique Bali';
...
© ${new Date().getFullYear()} PJ Antique Bali by CV Karya Bersama.
`;
}
 ...
 * Example: "Round Coffee Table — Ø 60cm × H 40cm | PJ Antique Bali"
 *
 * If dimensions are absent, falls back to SKU:
 * Example: "Round Coffee Table — LR-023 | PJ Antique Bali"
 */
function buildTitle(product) {
  const name    = product.name || 'Petrified Wood Piece';
  const sku     = product.id || '';
  return sku
    ? `${name} — SKU ${sku} | Petrified Wood by ${BRAND_NAME}`
    : `${name} | Petrified Wood by ${BRAND_NAME}`;
}
 ...
 *   Handcrafted by PJ Antique Bali, Ubud Bali.
 */
